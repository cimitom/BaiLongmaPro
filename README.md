...
**v0.4.94** | 一个持续运行、能记忆、能发声、能接入微信群和本地工具的数字意识实验框架。
...
`v0.4.94` 将默认源码启动改为 LAN 模式：`npm start` 和 `npm run start:backend` 会监听 `0.0.0.0`，允许同一局域网私有地址访问，并在启动时打印可访问的 Brain UI 地址。
